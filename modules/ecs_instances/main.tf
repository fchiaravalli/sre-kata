...
resource "aws_security_group" "instance" {
  description = "sg for ECS instances in ${var.environment}"
  vpc_id      = var.vpc_id
...
resource "aws_launch_configuration" "launch" {
  ...
  security_groups = concat([aws_security_group.instance.id],  var.additional_sg)
  user_data            = data.template_file.user_data.rendered
  iam_instance_profile = var.iam_instance_profile_id
  key_name             = var.key_name

  # aws_launch_configuration can not be modified.
  # Therefore we use create_buse_fleet = var.use_fleet efore_destroy so that a new modified aws_launch_configuration can be created 
